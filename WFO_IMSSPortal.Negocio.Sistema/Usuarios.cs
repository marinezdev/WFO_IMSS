...
namespace WFO_IMSSPortal.Negocio.Sistema
{
    public class Usuarios
    {
        ...
        public int SeleccionarValidarAcceso(string clave, string contra, IU.ManejadorSesion sesion, ref string mensaje)
        {
            List<string> resultado = new List<string>();

            var obtenido = usrs.UsuariosValidarAcceso(clave, contra, sesion, resultado);

            mensaje = resultado[1].ToString();
            return Funciones.Nums.TextoAEntero(resultado[0].ToString());
        }
